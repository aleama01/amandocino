...
const Story = ({ story, idx }: any) => {
  return (
    <div className='overflow-hidden text-[#edf0d884] hover:text-[#EDF0D8] duration-200'>
      <motion.div className='h-[30dvh] md:h-[300px] overflow-hidden w-auto flex flex-row justify-center items-center'
        ...
        <div className=' w-full h-full overflow-hidden' >
          <Image loading={idx < 3 ? 'eager' : 'lazy'} priority={idx < 3} fetchPriority={idx < 5 ? "high" : "low"} alt="Story post image"
            ...
            className='w-full h-full bg-cover object-cover object-center bg-center bg-no-repeat z-30 saturate-0 hover:saturate-100 brightness-50 hover:brightness-90 duration-200' src={`${story.image.url}`} />
        </div>
      </motion.div>
